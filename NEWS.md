...
- Added argument `idx` so that anything related to indices can be turned
  off (`idx = FALSE`)
...
- Workaround when SAM models recruitment age but there is no input catch
  data

- Added argument `proc_error_est` to turn off survival process error
  level
...
- Fixed bug when observations in the SAM model fit do not include years
  but instead an index for the years (1, 2, 3, …).

## Version 1.0.1

Changes to `SAM2FLStock`

- This function can now handle multi-fleet SAM model fits. An example is
  given in the documentation for this method (see `?SAM2FLStock`).
...
- New argument `spinoutyear` (logical `TRUE`/`FALSE`) to include/exclude
  estimates of biological data from SAM beyond last data year.
